...
def cornet_zv(data, dropout_rate, learning_rate=None):
    im, output = data[0]
    img = tf.keras.layers.Input(im.shape[1:], name="img")
    out = CORNetZV(num_classes=output.shape[-1], dropout=dropout_rate).forward(img)
    model = keras.models.Model(inputs=img, outputs=out)
    model.compile(optimizer=keras.optimizers.SGD(learning_rate=learning_rate), loss=logit_cost, metrics=["accuracy"])
    return model
